...
func startServer(cmd *cobra.Command, args []string) error {
	...
	client, err := createEcobeeClient(viper.GetString("ecobee.clientid"))
	if err != nil {
		return err
	}
...
func createEcobeeClient(clientId string) (*ecobee.Client, error) {
	if clientId == "" {
		return nil, fmt.Errorf("ecobee client id was not defined. Please run again with ECOBEE_CLIENTID defined")
	}
	return ecobee.NewClient(clientId, "auth-cache.json"), nil
}
